...
export default function Auth() {
  ...
  const handleSubmit = (e: React.FormEvent, provider?: string) => {
    e.preventDefault();
    // Pass role, name, and email to Dashboard via state.
    navigate('/dashboard', { state: { role: selectedRole, name, email } });
  };
            ...
            <div className="relative bg-[#2A2A2A]/50 p-1 rounded-lg">
              ...
              <div className="relative grid grid-cols-3 gap-1">
                {['Patient', 'Doctor', 'Hospital'].map((role) => (
                  <button
                    key={role}
                    onClick={() => setSelectedRole(role.toLowerCase() as 'patient' | 'doctor' | 'hospital')}
